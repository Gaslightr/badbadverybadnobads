...
function g:CreateBase(parent, title)
	if parent:IsA("Instance") then
		...
		base.main.Size = UDim2.fromScale(.4, .65)
...
function g:CreateSection(category, name)
	if type(category) == "table" then
		...
		section.ui_layout.Padding = UDim.new(0.04, 0)
...
function g:CreateComboBox(section, name, items, current_item)
	if type(section) == "table" and type(items) == "table" then
		...
		combo_box.container = Instance.new("Frame", combo_box.btn)
		combo_box.container.Size = UDim2.fromScale(1, 1)
		...
		function combo_box:change_item(i)
			combo_box.current_item = items[i]
			combo_box.btn.Text = combo_box.current_item
		end
